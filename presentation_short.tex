% !TeX document-id = {fc3ccb1e-aeba-4c77-9990-74eef049f67c}
% !TeX TXS-program:compile = txs:///pdflatex/[-output-directory=tmp]

%============================ TODO: UPDATE =====================================
% - Update the following dates, links
% - (Update 'pictures/erstiheft.pdf')
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\newcommand{\semester}{ winter term 25}
\newcommand{\dateCHF}{          (23.10.)}
%         ⚠ COMMENT OUT EVERY NOT-OCCURING EVENT STARTING LINE 250 ⚠
\newcommand{\dateSommerfest}{   (27.06.)}
\newcommand{\dateSpieleabend}{  (01.10., 15.10.)}
\newcommand{\dateGrillen}{      (10.10., 18.10.)}
%\newcommand{\dateStadtrallye}{  (20.10.)}
%\newcommand{\dateAbenteuerSchnitzeljagd}{21.04.}
\newcommand{\dateKneipentour}{  (07.10, 17.10.)}
\newcommand{\dateWanderung}{    (11.10., 25.10.)}
\newcommand{\dateFilmabend}{    (14.10.)} 
\newcommand{\dateFlunkyball}{   (22.10.)}
% \newcommand{\dateCTF}{          (22.05)}
% \newcommand{\dateKastenlauf}{   (23.04.)}
\newcommand{\dateBreakfast}{    (10.10.)}
\newcommand{\dateHuette}{       (07.11.)}
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\newcommand{\timeSitzung}{Thursdays 6:30 pm }
\newcommand{\locFachschaftszimmer}{C125 - Sand 14}
\newcommand{\locFSKammer}{C102 - Sand 14}
% \newcommand{\linkBBB}{https://bbb.fsi.uni-tuebingen.de/b/luk-v3t-dvk}
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\newcommand{\linkMaillists}{https://www.fsi.uni-tuebingen.de/infos/maillists/}
\newcommand{\linkErsti}{https://www.fsi.uni-tuebingen.de/du-bist-ersti/}
\newcommand{\linkAnmeldung}{https://eei.fsi.uni-tuebingen.de/}
\newcommand{\linkErstiheft}{https://teri.fsi.uni-tuebingen.de/anfiheft/}
\newcommand{\linkDiscord}{https://discord.gg/d4X2WjHWmQ}
\newcommand{\linkPresentation}{https://github.com/fsi-tue/anfipraesentation/releases}
\newcommand{\linkWebsiteInfo}{https://www.fsi.uni-tuebingen.de}
\newcommand{\linkWebsiteKogni}{https://www.fs-kogni.fsi.uni-tuebingen.de}
%===============================================================================

\documentclass[aspectratio=169]{beamer}

%-------------------------------------------------------------------------------
%   Edit colour scheme 
%-------------------------------------------------------------------------------



%===============================================================================

\beamertemplatenavigationsymbolsempty
\usepackage[utf8]{inputenc}
\usepackage{DejaVuSans}
\renewcommand*\familydefault{\sfdefault}
\usepackage[fixed]{fontawesome5}

\useoutertheme{infolines}
\useinnertheme[realshadow,corners=2pt,padding=2pt]{chamfered}
\usecolortheme{fsik}
\usepackage[english]{babel}
\usepackage{graphics, graphicx}
\usepackage{color}
\usepackage{latexsym}
\usepackage{etoolbox}
\usepackage{subfig}
\usepackage{tikz}
\PassOptionsToPackage{hyphens}{url}
\usepackage{hyperref}
\usepackage{url}
\usepackage{qrcode}
\usepackage{eurosym}

% Override these to reproduce a build, if you run
% "nix-build --argstr date YYYY-MM-DD" you don't need to change them manually:
\year=\year
\month=\month
\day=\day

% For a presenter mode viewer like 'pdfpc' invoked with 'make notes'
\newif\ifnotes
\input{makeconfig.tex}
\ifnotes
    \usepackage{pgfpages}
    \setbeameroption{show notes on second screen}
\fi


\newcommand<>{\hover}[1]{\uncover#2{%
		\begin{tikzpicture}[remember picture,overlay]%
		\draw[fill,opacity=0.4] (current page.south west)
		rectangle (current page.north east);
		\node at (current page.center) {#1};
		\end{tikzpicture}}
}

\setbeamertemplate{headline}{}

\definecolor{pblue}{rgb}{0.13,0.13,1}
\definecolor{pgreen}{rgb}{0,0.5,0}
\definecolor{pred}{rgb}{0.9,0,0}
\definecolor{pgrey}{rgb}{0.46,0.45,0.48}
\definecolor{darkgreen}{rgb}{0,0.5,0}
\definecolor{fsiblue}{HTML}{000080}
\definecolor{fsikturqoise}{HTML}{008080}
\definecolor{fskgreen}{HTML}{008000}

\title{Student Council Introduction}
\author{\textbf{fsi}}

\begin{document}

%------------------------------------------------------------------------------
%   Title
%------------------------------------------------------------------------------
\begin{frame}
	\centering
	\vspace{.5cm}
	\includegraphics[width=0.6\textwidth]{pictures/fsilogo_neu.pdf}\\
	% \includegraphics[width=0.6\textwidth]{pictures/fsk-logo.png}\\ % TODO: Richtiges PDF Logo
	\vspace*{0.5cm}
	\begin{Large}
		Hi! We help you $\{\text{into}\mid\text{in}\}$ your studies!
	\end{Large}

	\vspace{1cm}
	\semester
\end{frame}

%= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
%   Table of Contents
%   =================
%
%   1. This is us!
%   2. Was machen wir?
%   3. Mailinglisten
%   4. Wie lernt man Leute kennen
%       4.1 Ersti-Programm
%       4.2 Erstiheft
%       4.3 Discord
%   5. Mach doch mit!	
%= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =


%-------------------------------------------------------------------------------
%   Wer sind wir?
%-------------------------------------------------------------------------------

\section{This is us, the FSI!}
\begin{frame}{\insertsection}
	\begin{figure}
		\includegraphics[width=11cm]{pictures/fsi24.jpg}
	\end{figure}

	\note{Hier labern wir sympathisch alá ''Das da bin ich ohne Maske'',
		''Das ist der Tim'', ''Oh guck! Grace!'' usw.}
\end{frame}



%-------------------------------------------------------------------------------
%   Was machen wir? Wir vertreten euch!
%-------------------------------------------------------------------------------

\section{What we do?}
\begin{frame}{\insertsection}
	\begin{columns}
		\column{.5\textwidth}
		% \huge\itshape \textcolor{fsikturqoise}{Wir vertreten euch!}
		% \begin{frame}[<+->]
		\only<1>{\includegraphics[width=\linewidth]{pictures/MedienInfoStand.JPG}}
		\only<2>{\includegraphics[width=\linewidth]{pictures/FSI_pulli_hinten.JPG}}
		\only<3>{\includegraphics[width=\linewidth]{pictures/productiveFSIKWork.JPG}}
		\only<4>{\includegraphics[width=\linewidth]{pictures/FSIKzusammen.JPG}}
		\only<5>{
			\begin{minipage}{\linewidth}
				\centering
				\hspace{-1.5cm} % Bild weiter nach links verschieben
				\includegraphics[width=5cm]{pictures/clubhausfest.JPG} \\
				\vspace{0,01cm} % Vertikalen Abstand anpassen
				\hspace{1.5cm} % Unteres Bild mehr nach rechts schieben
				\includegraphics[width=5cm]{pictures/sommerfest.JPG}
				%  \includegraphics[width=5cm]{pictures/weihnachtsfeier.JPG}
			\end{minipage}
		}
		\only<6>{\includegraphics[width=\linewidth]{pictures/fsi24_lieb.jpg}}

		% \end{itemize}
		\column{.5\textwidth}
		\begin{itemize}[<+->]
			\item \textbf{Help/Council} for your problems
			      \note[item]{Bei Fragen und Problemen an die Fachschaft wenden}

			\item \textbf{Contact} to the faculty and staff
			      \note[item]{FBI erwähnen aka monatl. Treffen
				      \& FB Leitung ist uns aufgeschlossen}

			\item \textbf{Representation} in university politics and other student bodies
			      \note[item]{FBI erwähnen aka monatl. Treffen
				      \& FB Leitung ist uns aufgeschlossen}

			\item \textbf{Tools \& Services} like PPI, Student workshop (3D printers and such)

			\item \textbf{Events}
			      \faMusic Clubhausfest \dateCHF\ and
			      \faBeer Christmas Party (TBA)
			      % \faBeer Sommerfest \dateSommerfest
			      \note[item]{Neben Ansprechsperson sind wir für Alkoholausschank da}

			\item \textbf{Having fun!}

			      %            \item \textbf{Hochschulpolitik}: StuRa, FakRat
			      %            \item \textbf{Gremien} wie Studienkommission, Prüfungsausschuss,
			      %                  Berufungskommissionnen
			      %            \note[item]{Im PA sitzen Leute, die bei Härtefallanträgen auf deiner
			      %                        Seite sind. Evaluationen werden in Stuko besprochen,...}
			      %
			      %            \item \textbf{Tools \& Services} wie PPI, KKI, BBB, CodiMD
			      %            \note[item]{Alktklausuren auf \texttt{ppi.fsi.uni-...}, Kneipen auf 
			      %                        \texttt{kki.fsi.uni-...}, Eigene VideoConf auf 
			      %                        \texttt{bbb.fsi.uni-...}, Gemeinsame Notizen auf 
			      %                        \texttt{notes.fsi.uni-...}}
		\end{itemize}

	\end{columns}
\end{frame}


%-------------------------------------------------------------------------------
%   Mailinglisten
%-------------------------------------------------------------------------------

\section{Mailinglists}
\begin{frame}[t]{\insertsection}
	\textbf{First of:} Read your emails!
	\footnote{well, the important ones. Here the "How to filter Spam":
		\url{https://github.com/fsi-tue/workshops/blob/master/2019/05-8-unimails.pdf}}
	\begin{itemize}
		\item \textbf{info-studium}\\
		      \textcolor{red}{\faExclamationTriangle
			      This is often the only way for us and general computer science faculty to reach you. Registration is basically obligatory.
			      \faExclamationTriangle}
		\item \textbf{info-talk}\\
		      Computer Science related stuff, like events in and around the Tübingen area.
		\item \textbf{info-jobs}\\
		      Distributes paid job opportunities
	\end{itemize}
	\vfill
	\textbf{Signup:} \url{\linkMaillists}

	\note{Das hier ist \textit{arguably} die wichtigste Folie in der Präsentation.\\
		Die einzige Möglichkeit von der Anmeldung zum \textbf{Praktikum Mikrocomputer}
		und dem \textbf{Teamprojekt} zu erfahren. Letztere ist nur einmal im
		Jahr.\\
		Oder wenn der Prüfungsausschuss Informationen verschicken will.\\[2em]
		info-talk für Studenten-Kosmos und info-jobs Jobangebote, beide
		moderiert.}
\end{frame}

\begin{frame}{Signup immeadiately!}
	\begin{columns}
		\column{.7\textwidth}
		\begin{enumerate}
			\item Go to the site
			\item Enter your \textbf{email address}
			\item Click the \textbf{\textsc{REGISTRIEREN}} Button
		\end{enumerate}

		\centering
		\vspace{1cm}
		{\huge\color{fsiblue} \faHandsWash Thanks everyone! }

		\column{.3\textwidth}
		\centering
		\qrcode[height=.9\linewidth]{\linkMaillists}
		\\[1em]
		\url{\linkMaillists}
	\end{columns}
	\note{Hier jetzt Zeit geben, dass sich die Erstis anmelden können}
\end{frame}


%-------------------------------------------------------------------------------
%   Wie lerne ich leute kennen?
%-------------------------------------------------------------------------------

% \begin{frame}{Erstiheft}
% 	\begin{columns}
% 		\column{.7\textwidth}
% 		\begin{figure}
% 			\colorbox[HTML]{7ec850}{
% 				\includegraphics[height=.8\textheight]{pictures/erstiheft.pdf}} % TODO: Wo Bild
% 		\end{figure}
% 
% 		\column{.3\textwidth}
% 		\centering
% 		\qrcode[height=.9\linewidth]{\linkErstiheft}
% 		\\[1em]
% 		\url{\linkErstiheft}
% 	\end{columns}
% 
% 	\note{Hier kann auch sehr gut darauf hingewiesen werden, dass alle unsere
% 		Materialien online abrufbar sind.}
% \end{frame}

%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
% TODO: Might replace with generic version without dates
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{Ersti-Programm}

\begin{frame}{\insertsubsection}
	\begin{columns}
		\column{.7\textwidth}
		\begin{itemize}
			\large
			\item {\large\faDice} Game Night  \dateSpieleabend
			\item {\large\faBeer} Flunkyball   \dateFlunkyball
			\item {\large\faHiking} Hike  \dateWanderung
			\item {\large\faBeer} Pub Crawl  \dateKneipentour
			\item {\large\faUtensils} Cookout/BBQ  \dateGrillen
			      %\item {\large\faRoute} Stadtrallye \dateStadtrallye
			      % \item {\large\faMapMarker} Abenteuer Schnitzel-Jagd \dateAbenteuerSchnitzeljagd
			\item {\large\faFilm} Movie Night    \dateFilmabend
			      % \item {\large\faBeer} Kastenlauf   \dateKastenlauf
			\item {\large\faUtensils} Breakfast \dateBreakfast
			      % \item {\large\faFlag} Capture the Flag \dateCTF
		\end{itemize}
		\centering
		\vspace{1em}
		{\huge\color{fsikturqoise} Signup \faLongArrowAltRight}

		\column{.3\textwidth}
		\centering
		\qrcode[height=.9\linewidth]{\linkAnmeldung}
		\\[1em]
		\url{\linkAnmeldung}
	\end{columns}

	\note{Hier lässt sich darauf hinweisen, dass die Anmeldung zwecks Planbarkeit
		sehr gewünscht ist.}
\end{frame}



%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{Sonstiges}
\begin{frame}{Discord anyone?}
	\begin{columns}
		\column{.7\textwidth}
		\centering
		\textit{\Large Connect with your peers. Get to talk about assignmnents, hobbies and the like!}\\[1em]
		{\Huge\color{fsikturqoise}\faComments}

		%        
		\column{.3\textwidth}
		\centering
		\qrcode[height=.9\linewidth]{\linkDiscord}
		\\[1em]
		\url{\linkDiscord}
	\end{columns}

	\note{Kommunikation ist wichtig; Wurde von nem Studi eröffnet, wir haben
		uns da rangehangen; usw u so fort...}
\end{frame}


%-------------------------------------------------------------------------------
%   Mach doch mit!
%-------------------------------------------------------------------------------

\section{Join us!}
\begin{frame}{\insertsection}
	\begin{columns}
		\column{.5\textwidth}
		\Large
		{\Huge\color{fsikturqoise} \faClock[regular]}   \timeSitzung \\[.5em]
		{\Huge\color{fsiblue} \faMapMarker*}       \locFachschaftszimmer\\[.5em]
		{\Huge\color{fskgreen} \faMapMarker*}       \locFSKammer\\[.5em]
		\color{darkgray}
		\column{.5\textwidth}
		\centering
		\includegraphics[angle=90, width=4.5cm]{pictures/bene_liebt_fsi.JPG}
	\end{columns}
\end{frame}

%-------------------------------------------------------------------------------
%   Link zu dieser Präsentation
%-------------------------------------------------------------------------------

%\section{Diese Präsentation zum selber durchklicken}
%\begin{frame}{\insertsection}
%    \begin{columns}
%        \column{.5\textwidth}
%        \centering
%        \includegraphics[height=.8\textheight]{pictures/keepcalm.pdf}
%
%        \column{.5\textwidth}
%        \centering
%        \qrcode[height=.5\textheight]{\linkPresentation}
%        \\[1em]
%        \url{\linkPresentation}
%
%    \end{columns}
%\end{frame}

\section{More information}
\begin{frame}{\insertsection}
	\begin{columns}
		\column{.5\textwidth}
		\centering
		\textcolor{fsiblue}{\qrcode[height=.5\textheight]{\linkWebsiteInfo}
			\\[1em]
			\url{\linkWebsiteInfo}}

		\column{.4\textwidth}
		\centering
		\includegraphics[height=.8\textheight]{pictures/keepcalm.pdf} % TODO: Bild fehlt
		% \includegraphics[height=.8\textheight]{pictures/keepcalm_fsik2.pdf} % Wo

		% \column{.3\textwidth}
		% \centering
		% % replaced qr code kogni for Sommersemester with Info, cause no kognis in Summer
		% \textcolor{fskgreen}{\qrcode[height=.5\textheight]{\linkWebsiteKogni}
		% 	\\[1em]
		% 	\url{\linkWebsiteKogni}}


	\end{columns}
\end{frame}

\begin{frame}{This presentation}
	\centering
	\qrcode[height=.5\textheight]{\linkPresentation}\\[1em]
	\url{\linkPresentation}
\end{frame}

\end{document}
